...
def test_view(request):
    return HttpResponse(f"SECRET_KEY: {settings.SECRET_KEY} ; EMAIL_SENDER: {settings.EMAIL_HOST_SENDER}")
    ...
    if request.method == 'POST':
        ...
        if user_id is not None:
            with connection.cursor() as cursor:
                ...
                # Envoyer un email
                sender_email = settings.EMAIL_HOST_SENDER
                receiver_email = settings.EMAIL_HOST_RECEIVER
